# n8n 워크플로우 전체 분석 리스트 (100개)

> 분석일: 2026-02-07
> 대상 폴더: `n8n workflow/`
> 총 파일 수: **100개** (제외 없이 전수 포함)
...
- **노드 종류:** airtable, code, convertToFile, extractFromFile, googleDrive, googleSheets, httpRequest, if, manualTrigger, merge, set, slack, splitInBatches, stickyNote, wait, webhook
- **카테고리:** 컨텐츠자동화
- **핵심 기능:** 에어테이블 상품정보를 기반으로 나노바나나 AI가 상품 이미지를 실시간 생성하고 구글 드라이브에 업로드 후 슬랙으로 결과를 알리는 메인 시스템
...
- **카테고리:** 업무자동화
- **핵심 기능:** 다양한 트리거 유형을 모아둔 범용 워크플로우 시작점 템플릿
...
## #031
- **파일명:** `FC)데모단)견적서 계약서 자동 생성 시스템.json`
- **워크플로우 이름:** FC)데모단)견적서/계약서 자동 생성 시스템
- **노드 종류:** code, convertToFile, formTrigger, googleDrive, html, stickyNote
- **카테고리:** 업무자동화
- **핵심 기능:** 폼에 고객·상품 정보를 입력하면 금액을 자동 계산하고 견적서/계약서 HTML을 생성하여 구글 드라이브에 저장
...
- **노드 종류:** code, gmail, scheduleTrigger, slack
- **카테고리:** 업무자동화
...
- **외부 서비스:** SOLAPI(SMS), Shlink/dub.co(UTM), Tavily, Apify, Notion, GitHub
